...
public class Vision extends SubsystemBase {
    ...
    private Pose2d getPose() {
        ...
        return new Pose2d(pose.getX(), pose.getY(), Rotation2d.fromDegrees(Drivetrain.getInstance().getYaw()));
    }
    ...
    public Optional<Pose2d> getRobotPose() {
        ...
        if (tag != -1 && tag <= 8) {
            Pose2d pose1 = toFieldRelativeSpeeds(new Pose2d(pose.getX(), pose.getY(), Rotation2d.fromDegrees(Drivetrain.getInstance().getYaw())));
            Pose2d pose = new Pose2d(pose1.getX() + FieldPositions.aprilTags.get(tag).getX(),
                                     pose1.getY() + FieldPositions.aprilTags.get(tag).getY(),
                                     pose1.getRotation()
            );
    ...
    public static Pose2d toFieldRelativeSpeeds(double vxMetersPerSecond, double vyMetersPerSecond, double robotAngle1) {
        ...
        return new Pose2d(vxMetersPerSecond * robotAngle.getCos() - vyMetersPerSecond * robotAngle.getSin(),
