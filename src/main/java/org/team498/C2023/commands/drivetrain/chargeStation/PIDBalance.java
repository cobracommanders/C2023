...
public class PIDBalance extends CommandBase {
    ...
    public void execute() {
        ...
        if (Math.abs(angle) > deadzone) {
            isTipped = true;
            drivetrain.drive(Math.copySign(PID.calculate(gyro.getPitch()), -angle * Robot.coordinateFlip), 0, drivetrain.calculateRotationalSpeed(), true);
        } else if (!isTipped) {
            drivetrain.drive((-3 * Robot.coordinateFlip) * (invert ? -1 : 1), 0, drivetrain.calculateRotationalSpeed(), true);
        } else {
