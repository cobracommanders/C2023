...
public class SystemsCheck {
    private final ShuffleboardTab tab = Shuffleboard.getTab("Systems Check");
    ...
    public Command getCommand() {
        return sequence(
                ...
                test("IntakeWrist/Motors", () -> IntakeWrist.getInstance().setState(State.IntakeWrist.TEMPORARY_IDLE), () -> IntakeWrist.getInstance().atSetpoint(), 2),
                test("ElevatorWrist/Motor", () -> ElevatorWrist.getInstance().setState(State.ElevatorWrist.TRAVEL), () -> ElevatorWrist.getInstance().atSetpoint(), 2),
                test("Elevator/Motors", () -> Elevator.getInstance().setState(State.Elevator.TOP_CUBE), () -> Elevator.getInstance().atSetpoint(), 2),
                test("Manipulator/Motor", () -> Manipulator.getInstance().setState(State.Manipulator.INTAKE_CUBE), () -> Manipulator.getInstance().getCurrentDraw() > 0.2, 2),
    ...
    private CommandBase test(String system, Runnable test, BooleanSupplier isWorking, double timeout) {
        return sequence(
                runOnce(() -> {
                    timer.reset();
                    timer.start();
                    Blinkin.getInstance().setColor(BlinkinColor.SOLID_YELLOW);
                ...
                race(
                        sequence(
                                ...
                                runOnce(() -> {
                                    var element = system.split("/");
                                    var layout = tab.getLayout(element[0], "List Layout");
                                    layout.addBoolean(element[1],  () -> false);
                                }),
                        ...
                        sequence(
                                ...
                                runOnce(() -> {
                                    var element = system.split("/");
                                    var layout = tab.getLayout(element[0], "List Layout");
                                    layout.addBoolean(element[1],  () -> true);
                                }),
